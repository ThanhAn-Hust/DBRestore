    ...
    public void run(PromptReader reader) {
        ...
        if (!history.isEmpty()) {
            ...
            if (choice < history.size()) {
                backupId = history.get(choice).id();
            } else {
                backupId = reader.readString(i18n.getMessage("restore.enter_id"), "");
            }
        } else {
        ...
        DbConnectionConfig targetConn = null;
        ...
        if (targetChoice == 2) {
            ...
            if (!profileNames.isEmpty()) {
                ...
                if (pChoice < profileNames.size()) {
                    targetConn = profiles.get(profileNames.get(pChoice));
                } else {
                    targetConn = promptCustomConnection(reader);
                }
            } else {
                targetConn = promptCustomConnection(reader);
            }
